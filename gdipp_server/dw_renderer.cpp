...
IDWriteFactory *dw_renderer::_dw_factory = NULL;
IDWriteGdiInterop *dw_renderer::_dw_gdi_interop = NULL;
...
bool dw_renderer::begin(const dc_context *context, FT_Render_Mode render_mode)
{
	if (!renderer::begin(context, render_mode))
		return false;
...
bool dw_renderer::make_glyph_texture(FLOAT x, FLOAT y, const DWRITE_GLYPH_RUN *dw_glyph_run, glyph_run *a_glyph_run)
{
	HRESULT hr;
...
bool dw_renderer::render_glyph(LPCWSTR lpString, UINT c, glyph_run &new_glyph_run)
{
	HRESULT hr;
...
bool dw_renderer::render_text(LPCWSTR lpString, UINT c, glyph_run &new_glyph_run)
{
	HRESULT hr;
...
bool dw_renderer::render(bool is_glyph_index, bool is_pdy, LPCWSTR lpString, UINT c, CONST INT *lpDx, glyph_run &new_glyph_run)
{
	if (lpDx != NULL)
	{
...
IFACEMETHODIMP_(ULONG) dw_renderer::AddRef( void)
{
	return 0;
}

IFACEMETHODIMP_(ULONG) dw_renderer::Release( void)
{
	return 0;
}
