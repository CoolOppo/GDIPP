...
const os2_metrics *gdimm_font_man::lookup_os2_metrics(long font_id)
{
...
long gdimm_font_man::register_font(HDC font_holder, const wchar_t *font_face)
{
	...
	std::map<std::wstring, long>::const_iterator iter = _name_to_id.find(font_face);
...
long gdimm_font_man::link_font(const LOGFONTW &linked_font_attr, std::wstring &linked_font_face)
{
	error_status_t rpc_error;
	long font_id;
	...
	std::map<long, font_info>::const_iterator iter = _id_to_info.find(font_id);
