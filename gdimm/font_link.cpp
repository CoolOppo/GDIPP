...
{
	const map<t_string, vector<t_string>>::const_iterator iter = _link_table.find(font_name);
...
const size_t _gdimm_font_link::get_link_count(const TCHAR *font_name) const
{
	const map<t_string, vector<t_string>>::const_iterator iter = _link_table.find(font_name);
