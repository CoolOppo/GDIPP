...
const FT_Glyph gdimm_glyph_cache::lookup_glyph(uint64_t font_trait, FT_UInt index, bool is_glyph_index)
{
	...
	index_to_bmp_map::const_iterator index_iter = trait_iter->second.find(internal_index);
...
bool gdimm_glyph_cache::erase_font_trait(uint64_t font_trait)
{
	...
	map<uint64_t, index_to_bmp_map>::iterator bmp_trait_iter = _glyph_bmp_store.find(font_trait);
