...
_gdimm_text::_gdimm_text()
{
	...
	hr = dw_factory->CreateTextAnalyzer(&dw_analyzer);
	assert(SUCCEEDED(hr));
}
...
bool _gdimm_text::text_out(LPCWSTR lpString, UINT c, CONST RECT *lprect, CONST INT *lpDx)
{
	...
	IDWriteFont *dw_font;
	wcscpy(_font_attr.lfFaceName, get_font_family());
	hr = dw_gdi_interop->CreateFontFromLOGFONT(&_font_attr, &dw_font);
	assert(SUCCEEDED(hr));

	IDWriteFontFamily *dw_font_fam;
	hr = dw_font->GetFontFamily(&dw_font_fam);
	assert(SUCCEEDED(hr));

	IDWriteLocalizedStrings *dw_font_names;
	hr = dw_font_fam->GetFamilyNames(&dw_font_names);
	assert(SUCCEEDED(hr));

	UINT32 font_name_len;
	dw_font_names->GetStringLength(0, &font_name_len);
	WCHAR *font_name = new WCHAR[font_name_len + 1];
	dw_font_names->GetString(0, font_name, font_name_len + 1);
	...
	hr = dw_factory->CreateTextFormat(
		...
		dw_font->GetStyle(),
		dw_font->GetStretch(),
		_outline_metrics->otmTextMetrics.tmHeight - _outline_metrics->otmTextMetrics.tmInternalLeading,
		L"",
		&g_pTextFormat);
	assert(SUCCEEDED(hr));

	delete[] font_name;
	dw_font_names->Release();
	dw_font_fam->Release();
	dw_font->Release();
	...
	if (_font_attr.lfUnderline)
	{
		DWRITE_TEXT_RANGE textRange = {0, c};
		hr = g_pTextLayout->SetUnderline(true, textRange);
		assert(SUCCEEDED(hr));
	}

	if(_font_attr.lfStrikeOut)
	{
		DWRITE_TEXT_RANGE textRange = {0, c};
	...
	IDWriteBitmapRenderTarget* g_pBitmapRenderTarget;
	hr = dw_gdi_interop->CreateBitmapRenderTarget(_hdc_text, dc_bmp.bmWidth, dc_bmp.bmHeight, &g_pBitmapRenderTarget);
	assert(SUCCEEDED(hr));

	IDWriteRenderingParams* g_pRenderingParams;
	hr = dw_factory->CreateRenderingParams(&g_pRenderingParams);
	assert(SUCCEEDED(hr));

	IDWriteTextRenderer *g_pGdiTextRenderer = new gdimm_renderer(g_pBitmapRenderTarget, g_pRenderingParams, _eto_options & ETO_GLYPH_INDEX);

	const HDC memoryHdc = g_pBitmapRenderTarget->GetMemoryDC();

	BitBlt(memoryHdc, 0, 0, dc_bmp.bmWidth, dc_bmp.bmHeight, _hdc_text, _cursor.x, _cursor.y, SRCCOPY | NOMIRRORBITMAP);
	...
	BitBlt(_hdc_text,
		_cursor.x, _cursor.y,
		dc_bmp.bmWidth, dc_bmp.bmHeight,
		memoryHdc,
		0, 0, 
		SRCCOPY | NOMIRRORBITMAP);

	delete g_pGdiTextRenderer;
	g_pRenderingParams->Release();
	g_pBitmapRenderTarget->Release();
