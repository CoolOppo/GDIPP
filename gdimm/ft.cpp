...
void initialize_freetype()
{
	...
	FT_Library_SetLcdFilter(ft_lib, (FT_LcdFilter) gdimm_setting::instance().get_gdimm_setting<int>(TEXT("lcd_filter")));
...
FT_Error face_requester(FTC_FaceID face_id, FT_Library library, FT_Pointer request_data, FT_Face *aface)
{
	...
	args.stream = gdimm_font_man::instance().prepare_request((long) face_id);
