	...
	/* [in] */ int render_mode)
{
	...
	const gdipp::config_trait setting_trait(gdipp::get_gdi_weight_class(static_cast<unsigned short>(outline_metrics->otmTextMetrics.tmWeight)),
		!!outline_metrics->otmTextMetrics.tmItalic,
		point_size,
		metric_face_name(outline_metrics));
	new_session->font_setting = gdipp::get_gdimm_setting_cache(&setting_trait);
	...
	case gdipp::RENDERER_CLEARTYPE:
		break;
	default:
		new_session->renderer = new gdipp::gdimm_ggo_renderer(new_session);
		break;
	}
