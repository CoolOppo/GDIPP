...
GDIPP_API void get_dir_file_path(HMODULE h_module, const WCHAR *file_name, WCHAR source_path[MAX_PATH])
{
	DWORD dw_ret;
	BOOL b_ret;
...
GDIPP_API inline bool is_font_excluded(const WCHAR *font_name)
{
	return setting_instance.is_font_excluded(font_name);
}

GDIPP_API inline LONG CALLBACK create_minidump(__in struct _EXCEPTION_POINTERS *ExceptionInfo)
{
	BOOL b_ret;
